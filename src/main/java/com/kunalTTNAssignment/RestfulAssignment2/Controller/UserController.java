...
public class UserController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<User> getUserById(@Parameter(name = "id", description = "User id", example = "1") @PathVariable Integer id) {
        return new ResponseEntity<>(userService.getUserById(id), HttpStatus.OK);
    }

    @Operation(summary = "deleteUser", description = "Delete a user using XML")
    @ApiResponses(value = {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<String> deleteUserById(@Parameter(name = "id", description = "User id", example = "1") @PathVariable Integer id) {
